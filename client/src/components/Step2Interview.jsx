...
function Step2Interview({ interviewData, onFinish }) {
    ...
    return (
      <div className='min-h-screen bg-gray-50 dark:bg-slate-950 flex items-center justify-center p-4 transition-colors duration-300'>
        ...
        >
          ...
          <p className='text-gray-500 dark:text-gray-400 mb-8'>Role: {interviewData.role}</p>
          <div className='relative w-24 h-24 mx-auto flex items-center justify-center bg-gray-50 dark:bg-slate-800 rounded-full border-4 border-gray-100 dark:border-slate-700'>
            <span className='text-4xl font-bold text-black dark:text-white'>{entryCountdown}</span>
          </div>
          <p className='text-sm text-gray-400 dark:text-gray-500 mt-6'>Please prepare yourself in a quiet environment.</p>
        </motion.div>
      </div>
    );
  }
    ...
    return (
      <div className='min-h-screen bg-gray-50 dark:bg-slate-950 flex items-center justify-center p-4 transition-colors duration-300'>
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className='max-w-lg w-full bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-gray-200 dark:border-slate-800 p-8 transition-colors'
        >
          <h2 className='text-2xl font-bold text-gray-900 dark:text-white mb-6 text-center'>System Check</h2>
          <div className='space-y-4 mb-8'>
            <div className='flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700'>
              <div className='flex items-center gap-3'>
                <FaMicrophone className='text-gray-400 dark:text-gray-500' />
            ...
            {systemChecks.mic === 'failed' && (
              <div className='text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-4 rounded-xl border border-red-200 dark:border-red-900/50'>
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-50 via-white to-gray-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 flex items-center justify-center p-4 sm:p-6 transition-colors duration-300'>
      <div className='w-full max-w-6xl min-h-[85vh] bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-gray-200 dark:border-slate-800 flex flex-col lg:flex-row overflow-hidden transition-colors duration-300'>
        ...
        <div className='w-full lg:w-[35%] bg-white dark:bg-slate-900 flex flex-col items-center p-6 space-y-6 border-r border-gray-200 dark:border-slate-800 transition-colors'>
          <div className='w-full max-w-md rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-slate-800 relative'>
            <video
              src={videoSource}
              key={videoSource}
              ref={videoRef}
            ...
            <button onClick={toggleGender}
              className="absolute top-3 right-3 bg-white/80 dark:bg-slate-800/80 hover:bg-white dark:hover:bg-slate-700 text-gray-800 dark:text-gray-200 backdrop-blur px-3 py-1.5 rounded-full text-xs font-bold shadow-sm transition">
          ...
          <div className='w-full max-w-md bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl shadow-sm p-6 space-y-5 transition-colors'>
            ...
            <div className='grid grid-cols-2 gap-6 text-center'>
              <div>
                <span className='text-2xl font-bold text-black dark:text-white'>{currentIndex + 1}</span>
                <span className='text-xs text-gray-400 dark:text-gray-500 block mt-1 uppercase tracking-wider'>Current</span>
              </div>
              <div>
                <span className='text-2xl font-bold text-gray-400 dark:text-gray-500'>
                  {isInterviewFinished ? dynamicQuestions.length : "∞"}
                </span>
                <span className='text-xs text-gray-400 dark:text-gray-500 block mt-1 uppercase tracking-wider'>Total</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className='flex-1 flex flex-col p-6 sm:p-8 md:p-10 relative bg-gray-50/50 dark:bg-slate-900/50 transition-colors'>
          ...
          {!isIntroPhase && (
            <motion.div
              ...
              className='relative mb-8 bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm transition-colors'
            >
              <div className='flex justify-between items-center mb-4'>
                <div className='inline-block px-3 py-1 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 rounded-full text-xs font-semibold'>
              ...
              <div className={`text-lg sm:text-xl font-bold text-gray-900 dark:text-white leading-relaxed ${isPaused ? 'blur-sm opacity-50 select-none' : ''}`}>
          ...
          {!feedback ? (
            <div className='flex items-center gap-4 mt-6'>
                <div className='flex items-center gap-2'>
                  ...
                  {isMicOn && (
                    <div className="flex gap-1 items-center h-14 px-4 bg-gray-100 dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700" title="Audio Visualizer">
                      {[...Array(6)].map((_, i) => (
                        <motion.div
                          key={i}
            ...
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className='mt-6 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 p-6 rounded-2xl shadow-sm transition-colors'
            >
              <h4 className='text-xs uppercase tracking-wider text-gray-400 dark:text-gray-500 font-bold mb-2'>Interviewer Feedback</h4>
              <p className='text-gray-800 dark:text-gray-200 font-medium mb-6 leading-relaxed'>{feedback}</p>
