...
function Auth({ isModel = false, onClose }) {
    ...
    return (
        <div className={`w-full flex justify-center ${isModel ? "py-2" : "min-h-screen bg-[#f3f3f3] dark:bg-slate-900 items-center px-6 py-20"}`}>
            <motion.div
                initial={{ opacity: 0, y: -20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, ease: "easeOut" }}
                className={`w-full ${isModel ? "max-w-md p-6 sm:p-8 rounded-3xl" : "max-w-md p-8 sm:p-10 rounded-[32px]"} bg-white dark:bg-slate-800 shadow-2xl border border-gray-100 dark:border-slate-700 overflow-hidden relative transition-colors`}>
